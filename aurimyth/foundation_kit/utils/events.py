...
class EventBus:
    ...
    使用示例:
        ...
        async def on_user_created(event: UserCreatedEvent):
            print(f"User {event.username} created!")
    ...
    def subscribe(
        ...
        Returns:
            Union[EventHandler, Callable]: 处理器或装饰器
        """
    ...
    def get_history(
        self,
        event_type: Optional[Type[EventType]] = None,
        limit: int = 100,
    ) -> List[Event]:
        """获取事件历史。
    ...
    def get_handler_count(self, event_type: Optional[Type[EventType]] = None) -> int:
        """获取处理器数量。
