...
from aurimyth.foundation_kit.response import ResponseBuilder
...
class ErrorDetail(BaseModel):
    ...
    field: Optional[str] = Field(None, description="错误字段（仅字段验证错误）")
    location: Optional[str] = Field(None, description="错误位置（如：body, query, path）")
...
class BaseExceptionHandler(ErrorHandler):
    """自定义基础异常处理器。"""
    ...
    async def handle(self, exception: BaseException, request: Request) -> JSONResponse:
        """处理自定义异常。"""
...
class DatabaseExceptionHandler(ErrorHandler):
    """数据库异常处理器。"""
...
class ErrorHandlerChain:
    ...
    def _build_chain(self) -> ErrorHandler:
        ...
        """
        # 按优先级顺序构建处理链
        base_handler = BaseExceptionHandler()
