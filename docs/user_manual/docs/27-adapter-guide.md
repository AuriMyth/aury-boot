...
Adapter 模块用于封装第三方接口（如支付、短信、微信等外部服务）的调用，支持在不同环境中灵活切换真实调用与 Mock 实现，解决测试环境中第三方接口不可用的问题。

## 核心特性

- **多模式支持**：`real`（真实调用）、`sandbox`（沙箱环境）、`mock`（本地 Mock）、`disabled`（禁用）
- **方法级覆盖**：同一 Adapter 的不同方法可以使用不同模式
- **装饰器式 Mock**：使用 `.mock` 链式方法定义 Mock 逻辑
- **调用记录**：测试时可追踪所有调用历史
- **钩子机制**：支持调用前、调用后、异常处理钩子
...
# {"success": True, "transaction_id": "mock_tx_order_001", "amount": 100}
```
...
```python
import pytest
from myapp.adapters.payment_adapter import PaymentAdapter
from aury.boot.application.adapter import AdapterSettings
...
from aury.boot.application.adapter import (
    # 基类
    BaseAdapter,      # Adapter 基类
    HttpAdapter,      # HTTP Adapter 基类

    # 装饰器
    adapter_method,   # 方法装饰器

    # 配置
    AdapterSettings,  # Adapter 配置
...
from aury.boot.application.adapter import (
    ...
    AdapterSettings,
    AdapterMode,        # 模式枚举
    MockStrategy,       # Mock 策略枚举
    ThirdPartySettings, # 环境变量配置
...
**总结**：Adapter 模块让第三方 API 集成更加可控，通过环境切换和方法级模式覆盖，可以在不同环境中灵活使用真实调用或 Mock 实现，大幅提升开发和测试效率。
